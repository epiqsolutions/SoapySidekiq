...
rx_lo_execution_times = []
transmit_execution_times = []
...
def transmit(sdr, tx_stream, buff_len, tx_buff, rate):
    global running, tx_total_samps, tx_total_blocks, transmit_execution_times 
    ...
    while running:
        try:
            ...
            transmit_execution_times.append(end_time - start_time)

            tx_total_blocks += 1

            if rc.ret != buff_len:
                SoapySDR.log(SoapySDR.SOAPY_SDR_ERROR, 
                             'TX Error {}: {}'.format(rc.ret, SoapySDR.errToStr(rc.ret)))
...
def change_rx_channel(sdr, rx_handle):
    global ch_index, rx_lo_execution_times 

    ch_index = ((ch_index + 1) % 6) 
    if ch_index == 0:
        ch_index = 1

    freq = ch_index * 1000e6

    print(f"ch_index {ch_index}, freq {freq}")

    start_time = time.perf_counter()
    sdr.setFrequency(SOAPY_SDR_RX, rx_handle, freq)  
    end_time = time.perf_counter()

    rx_lo_execution_times.append(end_time - start_time)

def receive(sdr, rx_stream, num_blocks, buff_len, rx_buff, file_name, num_loops, rx_handle):
    global running, rx_total_samps, rx_total_blocks, tx_total_blocks
    ...
    if num_loops:
        while num_loops > 0:
            ...
            change_rx_channel(sdr, rx_handle)
...
def main(cardno, serial, topology,
         rx_handle, tx_handle, 
         rate, bw, freq, 
    ...
    if serial is None:
        ...
        args = dict(driver="sidekiq", serial=serial)
    ...
    fullscale = int(sdr.readSetting("full_scale"))
    ...
    receive_thread = threading.Thread(target=receive, name='receive',args=(sdr,rx_stream,num_blocks,rx_buff_len,rx_buff,file_name,num_loops,rx_handle,  ))
    ...
    while running:
        try:
            ...
            # Format with leading zeros and commas, handling width properly
            print(f"\nrx_total_samps: {rx_total_samps:11,d}," 
                  f"\trx_total_blocks: {int(rx_total_blocks):6,d},"
                  f"\trx samp/sec: {int(rx_total_samps / time_ctr):10,d}")

            print(f"tx_total_samps: {tx_total_samps:11,d}," 
                  f"\ttx_total_blocks: {int(tx_total_blocks):6,d},"
                  f"\ttx samp/sec: {int(tx_total_samps / time_ctr):10,d}\n")
    ...
    average_time = sum(rx_lo_execution_times) / len(rx_lo_execution_times)
    max_time = max(rx_lo_execution_times)

    print(f"\nAverage rx_lo execution time: {average_time:.6f} seconds")
    print(f"Maximum rx_lo execution time: {max_time:.6f} seconds\n")

    average_time = sum(transmit_execution_times) / len(transmit_execution_times)
    max_time = max(transmit_execution_times)

    print(f"\nAverage transmit execution time: {average_time:.6f} seconds")
    print(f"Maximum transmit execution time: {max_time:.6f} seconds\n")
