...
TEST_CASE("channel map preserves non-contiguous SDK handles")
{
    ChannelMap<int, int> channels({4, 9, 12}, {3, 8});

    REQUIRE_EQ(channels.rxCount(), std::size_t{3});
    REQUIRE_EQ(channels.txCount(), std::size_t{2});
    REQUIRE_EQ(channels.rxHandle(0), 4);
    REQUIRE_EQ(channels.rxHandle(2), 12);
    REQUIRE_EQ(channels.txHandle(0), 3);
    REQUIRE_EQ(channels.txHandle(1), 8);
}

TEST_CASE("channel map rejects invalid RX and TX channels")
{
    ChannelMap<int, int> channels({4}, {7});

    REQUIRE_THROWS_AS(channels.rxHandle(1), std::out_of_range);
    REQUIRE_THROWS_AS(channels.txHandle(1), std::out_of_range);

    ChannelMap<int, int> empty;
    REQUIRE_THROWS_AS(empty.rxHandle(0), std::out_of_range);
    REQUIRE_THROWS_AS(empty.txHandle(0), std::out_of_range);
}
